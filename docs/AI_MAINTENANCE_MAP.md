...
same-Guide replacement, and revocation - adds two dormant service-role-only entry
functions over the banked capacity, lock, lifetime, Guide/Practice eligibility, and
audit contracts. None of these slices adds an app caller, provider delivery,
...
- Dormant invitation foundations through `PRJ01_F-WS06-WI008-S02C` - shared invited-identity provisioning-helper generalization - are banked through synchronized app commit `1dd85b5`. `PRJ01_F-WS06-WI008-S02D` adds the separately reviewed dormant Guide-to-Explorer issuance/replacement/revocation functions over that substrate. None has an application caller or real-user path.
