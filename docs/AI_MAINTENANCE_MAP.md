...
| Session creation and supersession (proposed dormant DEF5-S4) | `supabase/migrations/20260716000000_user_session_creation_function.sql`; `supabase/tests/user_session_creation_*_test.sql` | Review-only service-role-only primitive consuming committed account-bound `login` or `role_reentry` redemption evidence. It proposes protected freshness policy, account-wide active-session and per-challenge uniqueness, safe exact retry, atomic supersession, and embedded Family B session audit. It has no caller, route, cookie, provider action, provisioning path, cloud action, or real-user activation. |
...
- The runtime login/provisioning write path, including any caller of the proposed dormant session primitive and all provider-identity/provisioning writes.
- AI chat persistence
- Reflection storage
