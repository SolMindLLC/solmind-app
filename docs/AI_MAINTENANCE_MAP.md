...
| Invitation acceptance foundations (`PRJ01_F-WS06-WI008-S02B` and `PRJ01_F-WS06-WI008-S02C` banked) | `supabase/migrations/20260718000000_authorizing_evidence_consumption.sql`; `20260718001000_invitation_acceptance_preparation.sql`; `20260718002000_guide_invitation_acceptance.sql`; `20260718003000_admin_guide_invitation_issuance.sql`; `20260721000000_explorer_engagement_capacity_foundation.sql`; `20260724000000_invited_identity_provisioning_helper_generalization.sql`; related `supabase/tests/*invitation*` and `invited_identity_provisioning_helper_explorer_test.sql` | Banked dormant evidence-consumption, pre-provider reservation, Guide acceptance, Admin Guide-invitation, Explorer capacity/lock foundations, and the shared invited-identity provisioning-helper generalization. `PRJ01_F-WS06-WI008-S02C` preserves the Guide path and adds only the dormant Explorer profile branch; it is banked in synchronized app commit `1dd85b5` with documentation commit `5e65025` after clean 30-migration replay, focused 303 assertions, complete 25-file / 1,344-assertion database validation, lint, typecheck, network build, and Fable PASS with no Critical/High/Medium finding. No provider IO, caller, route, delivery, relationship, session, consent, cloud action, deployment, or real-user activation is included. |
...
- Dormant invitation foundations through `PRJ01_F-WS06-WI008-S02C` - shared invited-identity provisioning-helper generalization: shared authorizing-evidence consumption, Guide/Explorer preparation reservations, Guide acceptance, Admin Guide-invitation issuance/revocation, Explorer capacity/lock-key substrate, and the shared Guide/Explorer invited-identity helper. They are banked through synchronized app commit `1dd85b5` and have no application caller or real-user path.
...
- The runtime login/provisioning write path, including any caller of the banked dormant session primitive and all provider-identity/provisioning writes.
- AI chat persistence
- Reflection storage
