-- SolMind MVP0 PRJ01_F-WS06-WI008-S02C:
-- shared invited-identity provisioning-helper generalization.
-- This database-only foundation remains dormant until separately approved
-- invitation-acceptance callers and runtime paths are implemented.
--
-- Purpose:
--   - replace the transitional Guide-only provisioning helper with one
--     protected role-parameterized invited-identity helper;
--   - preserve the banked Guide acceptance behavior through that helper;
--   - add the dormant Explorer profile create/reuse/re-entry branch;
--   - remove the obsolete Guide-only helper after its caller is rewired.
--
...
alter type private.solmind_invited_identity_result
  add attribute profile_onboarding_changed boolean;

create function private.solmind_provision_invited_identity(
...
declare
  ...
  v_explorer_profile core.explorer_profile%rowtype;
  v_count integer;
  v_now timestamptz := pg_catalog.clock_timestamp();
begin
  v_result.profile_onboarding_changed := false;
  ...
  if v_count = 0 then
    ...
    if p_role_code = 'guide' then
      ...
      if v_role.role_status <> 'active'
         ...
         or v_guide_profile.setup_status not in (
      ...
      v_result.profile_id := v_guide_profile.guide_profile_id;
      v_result.profile_created := false;
    else
      if v_role.role_status <> 'active' then
        raise exception 'solmind_invited_identity_ineligible';
      end if;
      ...
      select profile.*
        into v_explorer_profile
        from core.explorer_profile profile
       where profile.user_account_id = p_existing_user_account_id;

      if v_explorer_profile.status <> 'active' then
        raise exception 'solmind_invited_identity_ineligible';
      end if;

      if v_explorer_profile.onboarding_status in ('paused', 'ended') then
        update core.explorer_profile
           set onboarding_status = 'consent_pending',
               updated_at = v_now
         where explorer_profile_id = v_explorer_profile.explorer_profile_id;
        v_result.profile_onboarding_changed := true;
      elsif v_explorer_profile.onboarding_status not in (
        'invited', 'contact_verified', 'consent_pending', 'intake_pending', 'active'
      ) then
        raise exception 'solmind_invited_identity_ineligible';
      end if;

      v_result.profile_id := v_explorer_profile.explorer_profile_id;
      v_result.profile_created := false;
    end if;
  else
    raise exception 'solmind_invited_identity_conflict';
...
alter function private.solmind_provision_invited_identity(
  uuid, uuid, text, text, text, text, text, text, uuid, text
) owner to postgres;
revoke all on function private.solmind_provision_invited_identity(
  uuid, uuid, text, text, text, text, text, text, uuid, text
) from public, anon, authenticated, service_role;

comment on function private.solmind_provision_invited_identity(
  uuid, uuid, text, text, text, text, text, text, uuid, text
) is
  'Protected shared invited-identity helper for Guide and Explorer entry. It creates or exactly validates account, contact, Supabase binding, role, and matching profile invariants; preserves prior Explorer profile information during approved re-entry; and reports per-entity inserts. It never reads or mutates invitations/evidence, writes audit, creates sessions/relationships/consent, or performs provider IO. It is not app-callable.';

create or replace function public.solmind_accept_guide_invitation(
...
drop function private.solmind_provision_invited_guide_identity(
