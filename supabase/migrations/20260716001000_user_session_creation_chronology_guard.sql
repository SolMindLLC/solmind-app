-- SolMind MVP0 DEF5-S4 correction: reject chronologically older redeemed evidence.
-- Purpose:
--   - preserve the banked exact response-loss retry path;
--   - prevent never-sessionized older/equal redemption evidence from superseding a session
--     already created from newer/equal evidence for the same account;
--   - preserve every existing grant, RLS, audit, caller, and runtime boundary.
-- Adds no table, index, policy, grant expansion, caller, route, cookie, provider action,
-- invitation/provisioning path, Guide assignment, cloud path, or real-user flow.
...
begin
  ...
  begin
    ...
    -- Evidence order, not invocation arrival order, decides whether a new login may
    -- supersede prior session history. The exact-retry branch above has already
    -- handled a session tied to this same challenge. For a never-sessionized
    -- challenge, require a strictly greater database ordering key than every
    -- challenge that has already created a session for the account. The primary
    -- key is redemption time; challenge UUID is the deterministic equal-time
    -- tie-breaker, so invocation arrival order cannot select the terminal winner.
