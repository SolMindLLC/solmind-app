...
describe("Suggested Waypoint Guide detail browser contract", () => {
  ...
  it.each([
    ...
    [{ ...draft, draft_or_pending_destination: "line one\nline two" }],
    [{ ...draft, draft_or_pending_destination: "bad\u2028separator" }],
    [{ ...draft, draft_or_pending_destination: "bad\ud800surrogate" }],
    [{ ...draft, draft_or_pending_why: "bad\u2029separator" }],
    [{ ...draft, draft_or_pending_arrival_signals: ["not normalized e\u0301"] }],
