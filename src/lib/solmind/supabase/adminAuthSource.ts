...
//         -> createServiceRoleRpcExecutor(client)
...
//     existing server-only family (serviceRoleClient.ts, serviceRoleRpcExecutor.ts,
//     serverEnv.ts, requestAuthClient.ts, composeRequestAuthContext.ts, adminRouteAccess.ts).
...
//     the auth query client already defines, now backed by the enumerated RPC executor.
...
// passes the real service-role RPC executor; tests pass a mock executor with a fixed
...
// here (via createServiceRoleClient), adapts it to the enumerated RPC executor, then
// delegates to the injectable assembler above. A missing/blank service-role env throws
// at construction; the /admin route composition root catches that and denies (fail
// closed, no detail leaked).
